...
class OAuth2ClientInfo:
    ...
    @staticmethod
    def from_oidc_endpoint(
        oidc_config_endpoint: str, client_id: str, scopes: list[str], client_secret: str
    ) -> "OAuth2ClientInfo":
        """
...
def load_oidc_config(odic_well_known_endpoint: str) -> dict:
    ...
    """
    config = load_json(odic_well_known_endpoint)
    return config
...
def get_auth_url(client_info: OAuth2ClientInfo, redirect_uri: str) -> str:
    ...
    """
    return (
        f"{client_info.authorization_url}"
